"""
Vehicle Purchase & Vehicle Ownership Event Pack — 5-Layer Semantic Architecture

File layout (mirrors property_purchase_and_house_acquisition event pack):
  rules/domains/property/vehicle_purchase_and_vehicle_ownership/
    ...
    outcome_quality.json                  — Layer 5: Vehicle quality classification
    calibration_schema.json               — Schema for calibration overlay
    calibration_overlay.json              — Empirical tuning (Layer 3 calibration)

Logic flow:
  Dasha rule fires -> broad vehicle purchase window opens
  Transit rule fires -> window becomes active/narrow
  Fast trigger fires -> exact timing narrowed to TARGET_DAY
  Classical pattern -> modifies confidence (natal promise of vehicle yoga)
  Outcome/quality -> classifies event (luxury_vehicle/transport_assets/multiple_vehicles/debt_heavy)
...
def get_dasha_rules():
    """Layer 1: Broad timing — Mahadasha/Antardasha gates for vehicle purchase."""
    return _load_json("dasha_rules.json")


def get_transit_rules():
    """Layer 2: Medium timing — Jupiter/Venus/Lagna lord transit activation."""
    return _load_json("transit_rules.json")


def get_fast_trigger_rules():
    """Layer 3: Exact timing — SBC benefic Janma Nakshatra pinpointing."""
    return _load_json("fast_trigger_rules.json")


def get_classical_patterns():
    """Layer 4: Structural — Natal promise patterns for vehicle yoga."""
    return _load_json("classical_patterns.json")


def get_outcome_quality():
    """Layer 5: Classification — Vehicle purchase mode/quality."""
