"""
Property Purchase & House Acquisition Event Pack — 5-Layer Semantic Architecture

File layout (mirrors medical/relocation/finance event packs):
  rules/domains/property/property_purchase_and_house_acquisition/
    ...
    outcome_quality.json                  — Layer 5: Property quality classification
    calibration_schema.json               — Schema for calibration overlay
    calibration_overlay.json              — Empirical tuning (Layer 3 calibration)

Logic flow:
  Dasha rule fires -> broad property purchase window opens
  Transit rule fires -> window becomes active/narrow
  Fast trigger fires -> exact timing narrowed to TARGET_DAY
  Classical pattern -> modifies confidence (natal promise of property yoga)
  Outcome/quality -> classifies event (house_purchase/land/construction/luxury/investment)
...
def get_dasha_rules():
    """Layer 1: Broad timing — Mahadasha/Antardasha gates for property purchase."""
    return _load_json("dasha_rules.json")


def get_transit_rules():
    """Layer 2: Medium timing — Jupiter/Saturn/Mars transit activation."""
    return _load_json("transit_rules.json")


def get_fast_trigger_rules():
    """Layer 3: Exact timing — SBC/conjunction pinpointing."""
    return _load_json("fast_trigger_rules.json")


def get_classical_patterns():
    """Layer 4: Structural — Natal promise patterns for property yoga."""
    return _load_json("classical_patterns.json")


def get_outcome_quality():
    """Layer 5: Classification — Property purchase mode/quality."""
